...
public class ManageAlarms implements RequestHandler<SNSEvent, String> {
	...
	public String handleRequest(SNSEvent input, Context context) {
		...
		String sender = CommonUtils.getSender("alarms",alarmName, ssmClient);
		ArrayList<String> recipients = CommonUtils.getRecipients("alarms",alarmName, ssmClient);

		String alarmUrl = "https://eu-central-1.console.aws.amazon.com/cloudwatch/home?region=" + region
				+ "#alarmsV2:alarm/" + alarmName;
		...
		String html_body = "<html>" + "<head></head>" + "<body>" + "<h1>A " + eventType + " event was received: "
				+ subject + ".</h1>"
				+ "<p>Please open this message in your browser in order to navigate to the target group. <p>"
				+ "<br>Timestamp: " + timestamp + "<br>" + "<br><strong>AlarmName:</strong> " + alarmName + "<br>"
				+ "<br>AlarmDescription: " + alarmDescription + "<br>NewStateValue: " + newStateValue
				+ "<br>NewStateReason: " + newStateReason + "<br>StateChangeTime: " + stateChangeTime + "<br></br>"
				+ "<p>Trigger:</p>" + "<ul>" + "<li>MetricName: " + metricName + "</li>" + "<li>ComparisonOperator: "
				+ comparisonOperator + "</li>" + "<li>Threshold: " + threshold + "</li>" + "</ul>" + "<div>"
				+ "<br> <a href=\"" + alarmUrl
				+ "\" style=\"margin:5px;background-color:#EB7035;float:left;border:1px solid #EB7035;border-radius:3px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:12px;line-height:44px;text-align:center;text-decoration:none;width:100px;-webkit-text-size-adjust:none;mso-hide:all;\">Alarm dashboard</a>"
				+ "<br></br>" + "</div>" + "</body>" + "<footer>"
